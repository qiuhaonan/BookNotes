...
void alloc_ctx(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
	...
	ctx->cycle_buffer = user_param->cycle_buffer; // system page size
	ctx->cache_line_size = user_param->cache_line_size;

	ALLOCATE(user_param->port_by_qp, uint64_t, user_param->num_of_qps);

	/*if noPeak == ON(1), tarr_size = 1; else tarr_size = iteration * QPs */
		...
		struct pingpong_dest *rem_dest)
{
	...
	for (i = 0; i < num_of_qps ; i++) { // first set parameters for each QP
		...
		for (j = 0; j < user_param->post_list; j++) { // then set parameters for the post_list of each QP
			...
			ctx->wr[i*user_param->post_list + j].wr_id   = i; // QP index

			if (j == (user_param->post_list - 1)) {
				ctx->wr[i*user_param->post_list + j].send_flags = IBV_SEND_SIGNALED; // only the last WR in the post_list will be signaled.
			...
			// set for inline if the msg is < inline size
			if ((user_param->verb == SEND || user_param->verb == WRITE) && user_param->size <= user_param->inline_size)
...
int run_iter_bw(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
	...
	tot_iters = (uint64_t)user_param->iters*num_of_qps; // example: 5000 * 1 = 5000
		...
		for (index =0 ; index < num_of_qps ; index++) {
			...
			while ((ctx->scnt[index] < user_param->iters || user_param->test_type == DURATION) && (ctx->scnt[index] - ctx->ccnt[index]) < (user_param->tx_depth) &&
				...
				if (user_param->post_list == 1 && (ctx->scnt[index] % user_param->cq_mod == 0 && user_param->cq_mod > 1)
					...
					#endif
							ctx->wr[index].send_flags &= ~IBV_SEND_SIGNALED; // unsignaled
					#ifdef HAVE_ACCL_VERBS
					}
					#endif
				}
				...
				ctx->scnt[index] += user_param->post_list; // accumulate WRs on the QP
				totscnt += user_param->post_list; // accumulate the total WRs ++++++++++++++++++++++++++++++++++++
				/* ask for completion on this wr */
				/*post_list == 1 && ( CQ moderation circle || (iteration type && the last WR) )
				  signal the WR if CQ moderatin circle is satisified or if the last WR is reached */
				if (user_param->post_list == 1 &&
					...
					#endif
							ctx->wr[index].send_flags |= IBV_SEND_SIGNALED; // signaled
					#ifdef HAVE_ACCL_VERBS
					}
					#endif
				}
					...
					for (i = 0; i < ne; i++) {
						...
						if (user_param->noPeak == OFF) {
							...
							if (totccnt >=  tot_iters - 1)
								user_param->tcompleted[user_param->iters*num_of_qps - 1] = get_cycles(); // record cycles for the last cqe +++++++++++++++++++
							else
								user_param->tcompleted[totccnt-1] = get_cycles(); // record cycles for non-last cqe +++++++++++++++++++++++++++++++
						}
