...
static void init_perftest_params(struct perftest_parameters *user_param)
{
	...
	user_param->cycle_buffer		= sysconf(_SC_PAGESIZE); // system page size
...
void print_report_bw (struct perftest_parameters *user_param, struct bw_report_data *my_bw_rep)
{
	...
	run_inf_bi_factor = (user_param->duplex && user_param->test_method == RUN_INFINITELY) ? (user_param->verb == SEND ? 1 : 2) : 1 ; // 1
	tsize = run_inf_bi_factor * user_param->size; // size
	num_of_calculated_iters *= (user_param->test_type == DURATION) ? 1 : num_of_qps; // iter * qps
	/*noPeak == ON, val = 1; noPeak == OFF, val = 0 */
	location_arr = (user_param->noPeak) ? 0 : num_of_calculated_iters - 1; // if noPeak==ON, 0, else iter * qps - 1 
	/* support in GBS format */
	format_factor = (user_param->report_fmt == MBS) ? 0x100000 : 125000000; // GBps = 10e9 bps / 8
